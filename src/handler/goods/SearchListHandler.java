...
public class SearchListHandler implements CommandHandler {
	...
	public String process(HttpServletRequest request, HttpServletResponse respose) throws Throwable {
		...
		int pageSize = 6;  	//한 페이지에 출력할 상품의 수
		...
		count = goodsDao.getSearchCount(keyword);
		...
		if(count > 0 ){
			...
			number = count - ( currentPage - 1 )* pageSize; //없어도 되나??
		...
		if( count != 0 ){
			ArrayList<GoodsDataBean> searchgoods = goodsDao.getSearchGoods( start, end, keyword );
			request.setAttribute("searchgoods", searchgoods);
		}
		
		
		return "/goods/searchList.jsp";
	}
