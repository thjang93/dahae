...
public class OrderListHandler implements CommandHandler{
	...
	public String process(HttpServletRequest request, HttpServletResponse respose) throws Throwable {
		...
		int pageSize = 10;  	//�븳 �럹�씠吏��뿉 異쒕젰�븷 �긽�뭹�쓽 �닔
		...
		if(id.equals("admin")) {
			...
			if( count != 0 ){
				ArrayList<OrderDataBean> orderList = OrderDao.getArticles(start, end);
				request.setAttribute("orderList", orderList);
			}
			
		}else { 
			//회원 일 때
			count = OrderDao.getCount(id);
			//�럹�씠吏��뿉 ���븳 �꽕�젙 : 10媛쒖쓽 湲��쓣 �븯�굹�쓽 �럹�씠吏�濡� 留뚮뱾湲�
			...
			if( count != 0 ){
				ArrayList<OrderDataBean> orderList = OrderDao.getArticles(start, end, id);
				request.setAttribute("orderList", orderList);
			}
